...
import { deletePost, findPost, findPublishedPost, insertPost, listPostsByCreator, postExists, toPublicPost, updatePost, type PostInput } from "../db/posts.ts";
...
export function postRoutes(app: Web<AppState>): void {
	app.get("/api/v1/posts", requireAuth(), async (ctx) => {
		...
		return ok(ctx, { posts: rows.map(toPublicPost) });
	});
	...
	app.post("/api/v1/posts", requireAuth(), async (ctx) => {
		...
		await insertPost(username, input, words, readTime(words));
	...
	app.get("/api/v1/posts/:slug", requireAuth(), async (ctx) => {
		...
		return ok(ctx, { post: toPublicPost(row) });
	});
	...
	app.put("/api/v1/posts/:slug", requireAuth(), async (ctx) => {
		...
		const words = wordCount(input.markdown);
		await updatePost(username, input, words, readTime(words), previous);
