...
export async function findEmbedCustomization(username: string): Promise<EmbedCustomization> {
	...
	return {
		...
		postsPerRow: Number(row.posts_per_row),
...
export async function saveEmbedCustomization(username: string, value: EmbedCustomizationInput): Promise<EmbedCustomization> {
	...
	if (isMysqlFamily) {
		await sql`INSERT INTO creator_embeds (username, show_title, show_description, show_search, show_social, show_author, show_date, show_read_time, date_format, posts_per_row, show_post_descriptions, show_share, custom_css, updated_at)
			VALUES (${username}, ${title}, ${description}, ${search}, ${social}, ${author}, ${date}, ${readTime}, ${value.dateFormat}, ${value.postsPerRow}, ${postDescriptions}, ${share}, ${value.customCss}, ${timestamp})
			ON DUPLICATE KEY UPDATE
				show_title = ${title}, show_description = ${description}, show_search = ${search}, show_social = ${social},
				show_author = ${author}, show_date = ${date}, show_read_time = ${readTime}, date_format = ${value.dateFormat}, posts_per_row = ${value.postsPerRow},
		...
		await sql`INSERT INTO creator_embeds (username, show_title, show_description, show_search, show_social, show_author, show_date, show_read_time, date_format, posts_per_row, show_post_descriptions, show_share, custom_css, updated_at)
			...
			ON CONFLICT (username) DO UPDATE SET
				...
				posts_per_row = excluded.posts_per_row,
